...
const Location: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      if (!mapRef.current) {
        console.error('카카오맵: 지도 컨테이너를 찾을 수 없습니다.');
        return;
      }

      try {
        console.log('카카오맵 초기화 시작...', { lat, lng });
    ...
    if (window.kakao && window.kakao.maps) {
      ...
      console.log('카카오맵 API를 기다리는 중...');
      // API 스크립트가 아직 로드되지 않은 경우 대기
      let attempts = 0;
      const checkKakaoMap = setInterval(() => {
        attempts++;
        if (window.kakao && window.kakao.maps) {
          console.log(`카카오맵 API 발견 (${attempts}번째 시도)`);
          ...
          console.error('카카오맵 API 로드 실패: 5초 이상 대기');
          clearInterval(checkKakaoMap);
        }
      }, 100);
